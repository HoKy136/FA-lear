            ...
            <div className="w-75 mw-100 m-auto mt-3 ">
                <form className="row justify-content-between mb-3" onSubmit={this.handleAdd}>
                    <div className="col-10">
                        <input value={this.state.inputs.job} onChange={this.handleInputChange} name="job" type="text" className="form-control" placeholder="Do Something!!!" ></input>
                    </div>
                    <div className="col-2">
    ...
    render() {
        return (
            <div onClick={this.props.handleClick} className={`alert alert-${this.props.data.isCompeted ? "success" : "primary"} alert-dismissible`}>
                ...
                <button onClick={this.props.handleRemove} type="button" className="btn btn-danger"  style={{float: 'right'}} >Xóa</button>
            </div>
        )
    }
}
